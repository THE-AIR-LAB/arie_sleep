...
export function buildResearchWorkflowSeed(_primaryAgent: string): CanvasDoc {
  ...
  // Task 8 re-scope: the agent-owned state/policy/reward canvases now render in
  // the side drawer's agent tier (State/Policy/Reward per agent), so drop them
  // here to avoid duplicate tabs. The bottom workflow drawer keeps only the
  // workflow overview plus any genuine pairwise interaction canvases.
  const KEEP = new Set(["workflow-overview"]);
  const canvases = __full.canvases.filter((c) => KEEP.has(c.id));
  return { ...__full, activeId: canvases[0]?.id ?? __full.activeId, canvases };
}
