/* Fallback seed content for the Financial Analyst (legal) studio. The live Model Setup is
...
export const POLICY_NODES: FlowNode[] = [
  { id: "s", type: "start", nt: "Start", text: "You are a calm, helpful analyst. You will be given the current conversation inputs plus an already-updated client state. Use the updated state to decide the next assistant step.", x: 300, y: 10, w: 250, h: 110 },
...
/* ── Chat copy (Research persona) ── */
export const SUGGESTIONS: { icon: string; label: string }[] = [
  { icon: "Memo", label: "Screen this company for idea generation" },
  { icon: "Book", label: "Is the valuation already priced in?" },
];
