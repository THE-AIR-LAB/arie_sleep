...
/**
 * Produces a short, descriptive title for a law-demo simulation run — a concise
 * summary of the client's legal matter (e.g. "Wrongful Termination, 90-Day EEOC
 * Deadline") instead of the generic "Improvised client". Given the (optional)
 * scenario the client played and the conversation so far, it returns a <=8-word
 * title.
 */
export async function POST(request: NextRequest) {
  ...
  const system =
    "You write a very short title summarizing a prospective client's legal matter, " +
  ...
  const user =
    (scenario ? `Scenario the client played: ${scenario}\n\n` : "") +
